...
public:
    ...
    //Mostafa added
    int Timeslot[10000];
    double CarrierTime[10000];
    bool SetValue;
    int Counter_Timeslot;
    int JammerprotocolValue; //  0 for idle, 2 for send, 1 for correct RX, 3 for corrupted
    double ProtocolRunTime;



    static LAddress::L2Type ListMacAddr[100];
    int Total_Node, Position[100][100], distance[100][100], Y_i[100], X_i[100];
    double Calculate_distance(int, int, int, int);
    void Read_Nodes_Positions();
    int Find_Nearest_nighbor(int);
    int Find_Multicast_nieghbors(int , int);

    void PrintState();
    void SetInitialValues();
    void SendupCollectedData();
protected:
...
protected:
    ...
    cMessage* timeout;
    cMessage* Record; //mostafa added
    cMessage* SendUpInCollectedData;
