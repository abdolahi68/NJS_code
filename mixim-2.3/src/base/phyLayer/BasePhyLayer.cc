...
//Mostafa add
double BasePhyLayer::LossModel(double x1, double x2, double y1, double y2)
{

    double X, Y, Distance=0, Result=0 ;
    X= x1-x2;
    Y=y1-y2;
    ...
    if(Distance <=110)
        Result=(1-  ( pow( (Distance/110) , 4) )/2    );

    else
        Result= ( pow( (220 -Distance) / 110 , 4) / 2);
...
double BasePhyLayer::Gen_Rand(double a ,double b)
{
...
void BasePhyLayer::initialize(int stage) {
    ...
    Packet_Loss_because_of_fading=false;//Mostafa add
    number_of_similtous_frame=0;
...
void BasePhyLayer::initializeAnalogueModels(cXMLElement* xmlConfig) {
    ...
    // std::string s("SimplePathlossModel"); //Mostafa Added
    ...
    for(cXMLElementList::const_iterator it = analogueModelList.begin();
        ...
        ev<<"Mostafa "<<"AnalogueModel \"" << name << "\" loaded." << endl;
...
void BasePhyLayer::handleAirFrame(airframe_ptr_t frame) {
    ...
    case START_RECEIVE:
        ...
        if(number_of_similtous_frame==0)
        {
            sendControlMsgToMac(new cMessage("Start Busy Channel", JammerBusyChannel));

        }

        number_of_similtous_frame++;
        ev<<" number_of_similtous_frame= "<<number_of_similtous_frame<<endl;
...
void BasePhyLayer::handleAirFrameStartReceive(airframe_ptr_t frame) {
    ...
    //Mostafa added
    if(Packet_Loss_because_of_fading==true || number_of_similtous_frame>1)
    {
...
void BasePhyLayer::handleAirFrameEndReceive(airframe_ptr_t frame)
{
    ...
    if(number_of_similtous_frame==0)
        Packet_Loss_because_of_fading=false;

    ev<<" number_of_similtous_frame= "<<number_of_similtous_frame<<"  Packet_Loss_because_of_fading ="<<Packet_Loss_because_of_fading<<endl;

    if(number_of_similtous_frame>0)
    {
        ev<<"Mostafa number_of_similtous_frame ERORRRRRRR"<<endl;
...
void BasePhyLayer::filterSignal(airframe_ptr_t frame) {
    ...
    ev<<"mostafa "<<"sendersPos x = "<<sendersPos.x<<"sendersPos y = "<<sendersPos.y<<endl;
    ev<<"mostafa "<<"receiverPos x = "<<receiverPos.x<<"receiverPos y = "<<receiverPos.y<<endl;

    //mostafa added
    double pathloss= LossModel(sendersPos.x, receiverPos.x, sendersPos.y, receiverPos.y);
    double Generate_a_random=  uniform(0,1);//Gen_Rand(0,1);// Mostafa
    ev<<"Generate_a_random"<<Generate_a_random<<"  pathloss="<<pathloss<<endl;
    if((Generate_a_random > pathloss ) || (frame->getBitLength()>1000 && frame->getBitLength()<8000)) // Mostafa change && -- to --> ||
    {
        Packet_Loss_because_of_fading=true;
        ev<<" Packet_Loss_because_of_fading = true "<<endl;
    }
